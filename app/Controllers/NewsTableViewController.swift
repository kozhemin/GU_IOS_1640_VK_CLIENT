...
class NewsTableViewController: UITableViewController {
    ...
    private func prepareSectionData(newsResponse: NewsResponse) -> [NewsSection] {
        ...
        for itemNews in newsResponse.newsItems.items {
            ...
            if itemNews.sourceId > 0 {
                // profile
                if let profile = newsResponse.profileItems.profiles.first(where: { $0.id == abs(itemNews.sourceId) }) {
                    authorTitle = "\(profile.lastName) \(profile.firstName)"
                    authorPhoto = profile.photoUrl
                }
            } else {
                // group
                if let group = newsResponse.groupItems.groups.first(where: { $0.id == abs(itemNews.sourceId) }) {
